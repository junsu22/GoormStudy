...
Original file is located at
    https://colab.research.google.com/drive/1Q8_2sczdORtNtrPv9maxVO94sxqDCIiz
"""
...
# 객체 생성 5.4 버전은 max_tokens 를 안받는다.
...
# GPT-5.4는 max_tokens 미지원 → max_completion_tokens 사용
llm_with_logprob = ChatOpenAI(
    temperature=0.1,
...
# 스트리밍 실행
for chunk in chain.stream({"question": "LangChain이란 무엇인가요?"}):
    if chunk.content:
...
prompt = ChatPromptTemplate.from_messages([
    ("system", "당신은 재무제표를 분석하는 금융 AI입니다. 핵심만 간결하게 설명하세요."),
    ("user", "회사의 재무제표를 보고 흥미로운 사실을 정리해주세요.")
])
