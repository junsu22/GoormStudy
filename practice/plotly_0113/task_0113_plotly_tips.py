...
fig_b.update_traces(opacity=0.7)
# 그래프 출력
...
# ==============================
def set_grid_style(fig, dark_mode=False):
    gridcolor = "rgba(255,255,255,0.15)" if dark_mode else None
    fig.update_xaxes(showgrid=True, showline=True, mirror=True, gridwidth=1, gridcolor=gridcolor)
    fig.update_yaxes(showgrid=True, showline=True, mirror=True, gridwidth=1, gridcolor=gridcolor)
    return fig

# 그래프 A : 축  / 그리드 설정
# (다크 템플릿이라 그리드는 은은하게 보이도록 색 조정)
fig_a = set_grid_style(fig_a, dark_mode=True)
# 그래프 B : 축 / 그리드 설정
fig_b = set_grid_style(fig_b)
