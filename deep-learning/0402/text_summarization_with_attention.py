# -*- coding: utf-8 -*-
"""text_summarization_with_attention.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/10DGnPm19wNppTWrzugARWW5bNTfR2680
...
drop_train = [i for i in drop_train if i < len(encoder_input_train)]
drop_test = [i for i in drop_test if i < len(encoder_input_test)]

drop_train_enc = [i for i in drop_train if i < len(encoder_input_train)]
drop_train_dec_in = [i for i in drop_train if i < len(decoder_input_train)]
drop_train_dec_tar = [i for i in drop_train if i < len(decoder_target_train)]

drop_test_enc = [i for i in drop_test if i < len(encoder_input_test)]
drop_test_dec_in = [i for i in drop_test if i < len(decoder_input_test)]
drop_test_dec_tar = [i for i in drop_test if i < len(decoder_target_test)]

filtered = [
    (e, d_in, d_tar)
    for e, d_in, d_tar in zip(encoder_input_train, decoder_input_train, decoder_target_train)
    if len(e) > 0 and len(d_in) > 0 and len(d_tar) > 0
]

encoder_input_train, decoder_input_train, decoder_target_train = zip(*filtered)
...
encoder_input_train = list(encoder_input_train)
decoder_input_train = list(decoder_input_train)
decoder_target_train = list(decoder_target_train)
